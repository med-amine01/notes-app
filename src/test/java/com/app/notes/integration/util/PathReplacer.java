...
public final class PathReplacer {
    ...
    private static String replaceStoredIdPlaceholder(String path, Optional<String> storedId) {
        return storedId.map(id -> path.replace("{storedId}", id)).orElseGet(() -> path.replace("{storedId}", ""));
    }

    private static String replaceHardcodedIds(String path, String entityType, Map<Long, Long> idMappings) {
        String result = path;
